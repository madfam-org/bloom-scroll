...
- Release the Enclii `ps` runtime-health parity patch through the distributed CLI; source-built Enclii at `madfam-org/enclii@f919192` has been verified against Bloom production, and CLI use from this checkout still requires `ENCLII_PROJECT=bloom-scroll`.
